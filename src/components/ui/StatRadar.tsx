...
export default function StatRadar({ className = "" }: { className?: string }) {
    // Increased sizes to fill the 650px viewbox
    const size = 700; // Increased size slightly to accommodate larger chart
    const center = size / 2;
    const outerRadius = 184; // Increased by ~15% (160 -> 184)
    const innerRadius = 81;  // Increased by ~15% (70 -> 81)

    // Generate Star Path (10 points: 5 outer, 5 inner)
        ...
        <div className={`relative ${className} flex flex-col items-center justify-center w-full`}>
            ...
            <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto overflow-visible max-w-full">
                ...
                >
                    ...
                    />
                    {/* Inner Decorative Lines (optional depth) */}
                    <path
                        d={starPath}
                        fill="none"
                        stroke="#9d50bb"
                        strokeWidth="1"
                        transform={`scale(0.7) translate(${center * 0.3}, ${center * 0.3})`} // Simple scaling hack or re-calc ideal
                ...
                {/* Stat Labels & Levels */}
                {stats.map((s, i) => {
                    // Increased label radius offset for better spacing
                    ...
                    let xOffset = 0;
                    if (s.angle > -90 && s.angle < 90) xOffset = 15; // Increased visual offset
                    if (s.angle > 90 && s.angle < 270) xOffset = -15; // Increased visual offset
                    ...
                    return (
                        ...
                        >
                            ...
                            <text
                                ...
                                transform={`rotate(-5, ${labelPos.x}, ${labelPos.y})`} // Slight tilt
                            >
                                {s.label}
                            </text>

                            {/* Level Indicators (Blocks) */}
                            {/* Render 5 blocks next to/below label */}
                            <g transform={`translate(${labelPos.x + (s.angle === -90 ? -30 : (s.angle < 90 && s.angle > -90 ? 0 : -60)) + xOffset}, ${labelPos.y + 16}) rotate(-5)`}>
                                {[1, 2, 3, 4, 5].map((level) => (
                                    <rect
                                        key={level}
                                        x={(level - 1) * 14} // Wider spacing (12 -> 14)
                                        y={0}
                                        width="10" // Larger blocks (8 -> 10)
                                        height="8" // Larger blocks (6 -> 8)
